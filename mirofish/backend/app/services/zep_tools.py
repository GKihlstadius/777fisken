    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out junk content containing question numbers (Q1-9)
                skip = False
                for d in '123456789':
...
class ZepToolsService:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        f"Zep {operation_name} attempt {attempt + 1} attempt(s) failed: {str(e)[:100]}, "
                        f"{delay:.1f}s, retrying..."
                    )
                    time.sleep(delay)
                    delay *= 2
                else:
                    logger.error(f"Zep {operation_name} after {max_retries} attempt(s) still failed: {str(e)}")
    ...
    def search_graph(
        ...
        Uses hybrid search (semantic+BM25) to search for related info in the graph.
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all graph edges (paginated, with time info)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Gets all graph edges, then filters out edges related to the specified node
    ...
    def get_entities_by_type(
        ...
        """
        logger.info(f"Getting {entity_type} type entities...")
    ...
    def get_entity_summary(
        ...
        Search for all info related to this entity and generate summary
    ...
    def get_simulation_context(
        ...
        Args:
            ...
            limit: Count limit per info category
    ...
    def insight_forge(
        ...
        Most powerful hybrid retrieval function, auto-decomposes queries and retrieves multi-dimensionally:
        ...
        # Get details for all related entities (no quantity limit, complete output)
        entity_insights = []
    ...
    def _generate_sub_queries(
        ...
        Decompose complex query into multiple independently searchable sub-queries
        """
...
Return a JSON format list of sub-queries."""
    ...
    def panorama_search(
        ...
        3. Categorize active and historical info
        
        This tool is suitable for scenarios needing to understand the full picture of events and track evolution.
    ...
    def interview_agents(
        ...
        Calls the real OASIS interview API, interviewing Agents running in the simulation:
        ...
        4. Calls /api/simulation/interview/batch for real interviews (dual-platform simultaneous interview)
        5. Integrates all interview results, generates interview report
        
        [Important] This feature requires simulation environment in running state (OASIS environment not closed)
        ...
        Args:
            ...
            interview_requirement: Interview requirement description (unstructured, e.g., "understand student perspectives on the event")
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "4. Answer each question by number, starting each answer with \"Q X:\" (X is the question number)\n"
        ...
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                clean_text = re.sub(r'Q\s*\d+[：:]\s*', '', clean_text)
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean JSON tool call wrapping from Agent replies, extract actual content"""
...
Selection criteria:
1. Agent's identity/profession is relevant to interview topic
...
Please select up to {max_agents} most suitable Agents for interview and explain the reasoning."""
        ...
        try:
            ...
            # Get selected Agents' complete info
            selected_agents = []
            valid_indices = []
