...
Uses step-by-step generation to avoid failures from generating overly long content:
...
### Basic principles (for reference only, adjust based on specific events and participant groups):
- Target users are Chinese, must follow Beijing time daily routines
- 0-5 AM almost no activity (activity coefficient 0.05)
- 6-8 AM gradually active (activity coefficient 0.4)
- 9 AM - 6 PM moderately active (activity coefficient 0.7)
- 7-10 PM is peak period (activity coefficient 1.5)
...
- **Important**: Example values below are for reference only, adjust time periods based on event nature and participant characteristics
  - For example: Student groups may peak at 21-23; media active all day; official institutions only during work hours
...
- reasoning (string): Brief explanation of configuration rationale"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time configuration should follow Chinese daily routines."
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        # Type mapping table (handles different LLM output formats)
        type_aliases = {
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                logger.warning(f"No agent of type '{poster_type}' found, using highest influence Agent")
                if agent_configs:
...
- **Schedule follows Chinese routines**: almost no activity 0-5 AM, most active 7-10 PM
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<active hours list, considering Chinese routines>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configuration should follow Chinese daily routines."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Rule-based single Agent config generation (Chinese daily routine)"""
