...
Optimizations:
1. Calls Zep retrieval to enrich node info
...
class OasisProfileGenerator:
    ...
    # Group/organization entity types (need representative account persona)
    GROUP_ENTITY_TYPES = [
    ...
    def __init__(
        ...
        if api_key:
            ...
            self.model_name = model_name or eff_model

        if not self.api_key:
            raise ValueError("LLM API key not configured (set PRIMARY_LLM_API_KEY or LLM_API_KEY)")
        ...
        if fb_key:
            self.fallback_client = OpenAI(api_key=fb_key, base_url=fb_url)
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search to get rich info related to entity
        
        Zep doesn't have a built-in hybrid search API, need to search edges and nodes separately then merge results.
        Uses parallel requests for simultaneous searching, improving efficiency.
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.debug(f"Zep edge search after  {max_retries} attempt(s) still failed: {e}")
            return None
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.debug(f"Zep node search after  {max_retries} attempt(s) still failed: {e}")
            return None
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich info retrieved via Zep hybrid search
        """
        context_parts = []
            ...
            try:
                ...
                if self.fallback_client and attempt == 0:
                    try:
                        ...
                        logger.warning(f"Fallback LLM also failed: {str(fb_e)[:80]}")
                last_error = e
                import time
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for public opinion simulation, maximally restoring known real-world situations. Must return valid JSON format, all string values must not contain unescaped newlines. Use Chinese."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, maximally restoring known real-world situations.
...
2. persona: Detailed persona description (2000 characters plain text), should include:
   ...
   - Stance/viewpoints (attitude towards topics, content that may provoke/move them)
   - Unique traits (catchphrases, special experiences, personal hobbies)
   - Personal memories (important part of persona, should introduce this individual's connection to events and their existing actions and reactions in events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate detailed social media account settings for institution/group entity, maximally restoring known real-world situations.
...
2. persona: Detailed account settings description (2000 characters plain text), should include:
   ...
   - Stance/attitude (official stance on core topics, handling of controversies)
   - Special notes (represented group profile, operational habits)
   - Institutional memories (important part of institution persona, should introduce this institution's connection to events and their existing actions and reactions in events)
3. age: Fixed at 30 (institution account virtual age)
4. gender: Fixed as "other" (institution accounts use other to indicate non-personal)
...
8. interested_topics: Areas of interest array
...
- Institution account speech must match its identity positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        """Generate basic persona using rule-based approach"""
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Write file in real-time (even for fallback persona)
                    save_profiles_realtime()
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Output generated persona to console in real-time (complete content, no truncation)"""
        separator = "-" * 70
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profiles as CSV format (complies with OASIS official requirements)
