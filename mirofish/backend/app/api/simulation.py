...
# Adding this prefix prevents Agent from calling tools, directly replying with text
...
def get_entity_detail(graph_id: str, entity_uuid: str):
    """Get single entity detailed info"""
    try:
        if not Config.ZEP_API_KEY:
            return jsonify({
                "success": False,
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - preparing: If config_generated=True, it's complete
...
def prepare_simulation():
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Completed preparation exists, no need for duplicate generation",
        ...
        # This way the frontend can immediately get expected Agent count after calling prepare
        try:
            ...
            # Save entity count to state (for frontend to immediately fetch)
            ...
            logger.warning(f"Synchronously fetching entity count failed (will retry in background task): {e}")
...
def get_prepare_status():
    ...
    1. Query progress of ongoing task via task_id
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    if multiple, return newest (sorted by created_at)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Real-time fetch simulation Agent Profiles (for viewing progress during generation)
    ...
    - Returns extra metadata (e.g., file modification time, whether currently generating)
    ...
    try:
        ...
        logger.error(f"Failed to real-time fetch profiles: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Real-time fetch simulation config (for viewing progress during generation)
    ...
    try:
        ...
        logger.error(f"Failed to real-time fetch config: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // Optional: Max simulation rounds, truncates overly long simulations
    ...
    About force parameter:
        ...
        - Will not clear config files (simulation_config.json) and profile files
        - Suitable for scenarios needing to rerun simulation
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                # Preparation work incomplete
                return jsonify({
                    "success": False,
                    "error": f"Simulation not prepared, current status: {state.status.value}，please call /prepare endpoint first"
                }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions only shows content from latest round on both platforms
...
def interview_agent():
    ...
    Note: This feature requires simulation environment in running state (after completing simulation cycle, enters wait-for-command mode)
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure simulation has completed and entered wait-for-command mode."
            }), 400
...
def interview_agents_batch():
    ...
    Note: This feature requires simulation environment in running state
