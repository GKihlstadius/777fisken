...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout streams to UTF-8 (fix console encoding)
...
# Monkey-patch camel-ai FunctionTool schema for Groq compatibility:
# Functions with no parameters (e.g. do_nothing) get 'required' without 'properties',
# which Groq rejects. This ensures valid JSON schema for parameterless tools.
def _patch_camel_function_tool_schema():
    try:
        from camel.toolkits.function_tool import sanitize_and_enforce_required as _orig
        def _patched(parameters_dict):
            result = _orig(parameters_dict)
        ...
        import camel.toolkits.function_tool as ftmod
        ftmod.sanitize_and_enforce_required = _patched
    except ImportError:
        pass

_patch_camel_function_tool_schema()
...
# Add filter immediately at module load time, ensuring it takes effect before camel code execution
...
class ParallelIPCHandler:
    """
    Dual-platformIPC command handler
    
    Manages both platform environments, processes interview commands
    """
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get the specified platform's environment and agent_graph
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        if success_count > 0:
            ...
            print(f"  Interview failed: agent_id={agent_id}, AllPlatform all Failed")
            return False
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        # ProcessTwitterPlatform Interview
        ...
        # ProcessRedditPlatform Interview
        ...
        if results:
            ...
            self.send_response(command_id, "failed", error="NoSuccess Interview")
            return False
...
# Filter out non-core action types (these actions have low analysis value)
FILTERED_ACTIONS = {'refresh', 'sign_up'}

# ActionType mapping table (database name -> standard name)
ACTION_TYPE_MAP = {
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from database with complete context info.
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keep only key fields (preserve complete content, no truncation)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    """
    Enrich action with context info (post content, usernames, etc.)
    ...
    try:
        ...
        # Quote post: add original post content, author, and quote comment
            ...
            # GetCitationPost Comment content(quote_content)
            if new_post_id:
                cursor.execute("""
        ...
        print(f"Enriching action context failed: {e}")
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM config for speedup during parallel simulation:
    ...
    if use_boost and has_boost_config:
        ...
        # Use general config — prefer PRIMARY_LLM_* with fallback to legacy LLM_*
        llm_api_key = os.environ.get("PRIMARY_LLM_API_KEY", "") or os.environ.get("LLM_API_KEY", "")
        llm_base_url = os.environ.get("PRIMARY_LLM_BASE_URL", "") or os.environ.get("LLM_BASE_URL", "")
        llm_model = os.environ.get("PRIMARY_LLM_MODEL", "") or os.environ.get("LLM_MODEL_NAME", "")
...
def get_active_agents_for_round(
    ...
    # max_agents_per_round: configurable in time_config, defaults to 1 for free-tier rate limits
    max_per_round = time_config.get("max_agents_per_round", 1)
    selected_ids = random.sample(
        candidates,
        min(max_per_round, len(candidates))
    ) if candidates else []
...
async def run_twitter_simulation(
    ...
    # Get real agent name mapping from config file (use entity_name instead of default Agent_X)
    ...
    for round_num in range(total_rounds):
        ...
        # Rate-limit delay: configurable in time_config, defaults to 15s for free-tier
        rate_limit_delay = config.get("time_config", {}).get("rate_limit_delay_seconds", 15)
        await asyncio.sleep(rate_limit_delay)

        # Fetch actually executed actions from database and record
...
async def main():
    ...
    # Store both platform simulation results
    ...
    if args.twitter_only:
        ...
        run_mode = config.get("time_config", {}).get("platform_run_mode", "sequential")
        if run_mode == "parallel":
            ...
            # Sequential: avoids free-tier rate limit conflicts on shared API keys
            twitter_result = await run_twitter_simulation(config, simulation_dir, twitter_logger, log_manager, args.max_rounds)
            reddit_result = await run_reddit_simulation(config, simulation_dir, reddit_logger, log_manager, args.max_rounds)
    ...
    if wait_for_commands:
        ...
        log_manager.info("Support Command: interview, batch_interview, close_env")
        log_manager.info("=" * 60)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean multiprocessing resource tracker (prevent warning on exit)
        try:
